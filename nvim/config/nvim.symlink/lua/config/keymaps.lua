...
-- 2016 habit restored: toggle relative line numbers.
map("n", "<C-n>", function() vim.wo.relativenumber = not vim.wo.relativenumber end, { desc = "Toggle Relative Line Numbers" })

-- 2016 habit restored (new): search for the visual selection, forward and
-- backward. Yank the selection, then search for its literal text (\V) from
-- the unnamed register.
map("x", "*", [[y/\V<C-r>"]<CR>]], { silent = true, desc = "Search Selection Forward" })
map("x", "#", [[y?\V<C-r>"]<CR>]], { silent = true, desc = "Search Selection Backward" })
